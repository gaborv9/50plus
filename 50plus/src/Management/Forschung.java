...
public class Forschung extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		for (Person ptemp : pser.getPersonList()) {
			...
			gesamtpost = gesamtpost
					+ postser.getPostNumbers(ptemp.getID()).size();
		}
		...
		int role = pser
				.getPersonbyid((String) session.getAttribute("username"))
				.getRole();
		if (role != 2) {
		...
		if (role == 2 && searchedperson == null) { // Ausgabe fuer allgemeine
			...
			if (check == 0) {
				...
				session.setAttribute("personfound", "false"); // Person wurde
																// nicht
																// gefunden
				response.sendRedirect("Forschung.jsp");
			} else {
				...
				for (Person freunde : pser.getPersonbyid(searchedperson)
						.getFreunde()) {
					freundcounter++; // Freundeanzahl
				}

				personwerte.add(postser.getOwnpostlist(searchedperson).size());// postanzahl
																				// des
																				// users
